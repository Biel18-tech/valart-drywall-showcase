          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-border">
              <h2 className="text-xl font-heading font-bold text-primary flex items-center gap-2">
                <ShoppingCart size={22} /> Meu Carrinho
              </h2>
                  ...
                  {items.map((item) => (
                    <div key={item.name} className="flex gap-3 bg-secondary rounded-lg p-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <h4 className="text-sm font-heading font-semibold text-foreground line-clamp-2">{item.name}</h4>
            ...
            {items.length > 0 && (
              <div className="border-t border-border p-5 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-heading font-black text-xl text-red-brand">R$ {total.toFixed(2).replace(".", ",")}</span>
                </div>
                <button onClick={handleCheckout}
                  className="w-full bg-red-brand hover:bg-red-brand-hover text-accent-foreground font-heading font-bold py-3.5 rounded-lg transition-colors text-base">
                  Finalizar Compra
                </button>
              </div>
            )}
